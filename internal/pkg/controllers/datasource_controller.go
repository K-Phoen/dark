...
type datasourcesManager interface {
	RawSpecToModel(rawJSON []byte) (datasource.Datasource, error)
	Upsert(ctx context.Context, model datasource.Datasource) error
	Delete(ctx context.Context, model datasource.Datasource) error
}
...
func StartDatasourceReconciler(ctrlManager ctrl.Manager, grabanaClient *grabana.Client) error {
	reconciler := &DatasourceReconciler{
		...
		Datasources: grafana.NewDatasources(grabanaClient),
	}
...
func (r *DatasourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	datasourceManifest := &k8skevingomezfrv1alpha1.Datasource{}
	...
	datasourceModel, err := r.Datasources.RawSpecToModel(datasourceManifest.Spec.Raw)
	if err != nil {
	...
	if datasourceManifest.ObjectMeta.DeletionTimestamp.IsZero() {
		...
		if containsString(datasourceManifest.GetFinalizers(), datasourcesFinalizerName) {
			...
			if err := r.Datasources.Delete(ctx, datasourceModel); err != nil {
...
func (r *DatasourceReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&k8skevingomezfrv1alpha1.Datasource{}).
		Complete(r)
}

func (r *DatasourceReconciler) updateStatus(ctx context.Context, datasource *k8skevingomezfrv1alpha1.Datasource, err error) {
	logger := log.FromContext(ctx)
